app.controller('ThisWeatherCtrl', function ($scope, $http) {
    $scope.ListWeatherForecast = function (namecity) {
        $http.get('http://api.openweathermap.org/data/2.5/forecast/?q=' + namecity + '&mode=json&lang=vi&units=metric&appid=3154889146ceeee3393126b168f4561a').then(function (response) {
            $scope.listWeather = response.data;
            var weather_text = $scope.listWeather.list[0].weather[0].main.toLowerCase();
            var weather_type = weather.filter(x=>x.type == weather_text)
            changeWeather(weather_type[0]);
        })
    }
    $scope.ListWeatherForecast();
